...
pub static mut CONCURRENTHASHMAPSLEEPLOCK: ConcurrentHashMapSleepLock =
    ConcurrentHashMapSleepLock::new();

// Concurrent hash map implementation.
pub struct ConcurrentHashMapSleepLock {
    buckets: Vec<Option<SleepLock<Entry<i32, i32>>>>,
    size: AtomicUsize,
    // size: usize,
    // size: TicketLock<usize>,
}

impl ConcurrentHashMapSleepLock
where
    i32: Eq + core::hash::Hash,
{
    ...
    pub fn init(&mut self, capacity: usize) {
        let mut buckets: Vec<Option<SleepLock<Entry<i32, i32>>>> = Vec::with_capacity(capacity);
        for _ in 0..capacity {
            buckets.push(None);
        }
    ...
    pub fn insert(&mut self, key: i32, value: i32) {
        ...
        if self.buckets[index].is_none() {
            self.buckets[index] = Some(SleepLock::new(
                Entry { key: 0, value: 0 },
    ...
    pub fn get(&self, key: &i32) -> Option<&i32> {
        ...
        if let Some(sleep_lock) = &self.buckets[index] {
            let guard = sleep_lock.lock();
            let entry = unsafe { &*guard.sleep_lock().get_mut() };
            if &entry.key == key {
